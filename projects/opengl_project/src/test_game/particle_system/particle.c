...
void particle_spawn(particle_t *particle, const particle_spawn_props_t props)
{
    ...
    particle->rotation_speed = props.rotation_speed;
...
void particle_set_batch_vertex(particle_t *p, float *vertex)
{
    ...
    //[ posx, posy, scale, colorr, colorg, colorb, colora ]
    ...
    {
        float x = pos[i][0] - p->pos[0];
        float y = pos[i][1] - p->pos[1];
        pos[i][0] = x * cos_rot - y * sin_rot + p->pos[0];
        pos[i][1] = x * sin_rot + y * cos_rot + p->pos[1];
    }
    ...
    for(size_t i = 0; i < 4; i++)
    {
        ...
        vertex[(i * stride) + 2] = size;
        vertex[(i * stride) + 3] = color[0];
        vertex[(i * stride) + 4] = color[1];
        vertex[(i * stride) + 5] = color[2];
        vertex[(i * stride) + 6] = color[3];
    }
}
