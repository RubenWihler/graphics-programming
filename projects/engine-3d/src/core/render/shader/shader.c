...
bool shader_init(shader_t *shader, const char* dirpath)
{
    ...
    {
        LOG_ERROR("Error while building shader: %s !", shader->dirpath);
        return false;
    }
...
static int shader_get_uniform_location(const shader_t *shader, const char* name)
{
    ...
    if(location == -1) LOG_ERROR("Cannot find uniform location : %s !", name);
...
static bool shader_build(shader_t *shader)
{
    ...
    if(!src)
    {
        LOG_ERROR("Cannot parse shader program source");
        return false;
    }
