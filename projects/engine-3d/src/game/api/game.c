...
int game_init(game_t *game, const game_config_t* config, game_init_fn init_fn){
    ...
    if(!gl_init(game))
    {
        LOG_ERROR_FATAL("game initialization failed!");
        game_clean(game);
        return false;
    }
    ...
    if(!init_fn(game))
    {
        LOG_ERROR_FATAL("child game initialization failed!");
        game_clean(game);
        return false;
    }
    ...
    {
        LOG_ERROR_FATAL("callback initialization failed!");
        game_clean(game);
        return false;
    }
...
static bool gl_init(game_t* game)
{
    ...
    if (!glfwInit())
    {
        LOG_ERROR_FATAL("Error while init GLFW!");
        return false;
    }
    ...
    if (!game->window)
    {
        LOG_ERROR_FATAL("Error while creating window!");
        glfwTerminate();
        return false;
    }
    ...
    {
        LOG_ERROR_FATAL("Failed to initialize GLAD. OpenGL context might not be active.\n");
        return false;
    }
